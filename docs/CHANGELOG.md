...
- **D-A11.2 (Matriz patrón Core canónico, F-CLEAN-MODULES-FIX 2026-05-11)**: tras verificación empírica de los 6 archivos Core en disco (Logger, TimeSync, PersistenceLayer, BigNumbers, ModuleRegistry, EventBus, AdminCommands) + device AdminPanel, el proyecto canoniza **3 patrones Core vigentes**. D-A11 original afirmaba que 5 Cores eran `class<concrete>:` singletons top-level — factualmente falso, ningún Core en disco usa ese patrón (ver errata D-A11). Esta decisión D-A11.2 reemplaza la afirmación arquitectónica de D-A11 sin invalidar su núcleo (EventBus excepción device sigue válido).
  ...
  **Anti-patrones confirmados** (no usar):
  - `Name := name_module{}` archetype top-level — afirmado D-A11 pre-corrección, NUNCA aplicado empíricamente. 0 Cores en disco usan este patrón.
  ...
  **Cross-refs**: `VERSE_SYNTAX_GUIDE.md` §2.1 (Stateless namespace), §2.4 (PersistenceLayer top-level scope), §2.4-bis (AdminCommands Core+Device state-bearing). Justificación lecciones 5/14/16. `MODULES_DEPENDENCY_GRAPH.md` §2.1 inicio (mismo cuadro replicado como referencia de §4).

  **Bucket**: 1 (matriz transferible al handbook futuro) + 2 (ejemplos específicos RPG).
