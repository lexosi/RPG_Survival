...
### EventBus
Singleton top-level **generado** que expone instancias `event(payload_t)` nativas de Verse, una por cada evento del catálogo `data/architecture/events_catalog.json`. Archivo source manual: `Content/Verse/Core/EventBus.verse` (placeholder/legacy si se conserva). Archivo operativo: `Content/Verse/Generated/EventBusConstants.verse` (generado por Python). SYS-072. SPR-009. **Comunicación type-safe compile-time**: cada evento tiene struct payload generado en `Generated/EventPayloads_Generated.verse`. Cambios en payloads detectados por compilador (no más bugs silenciosos). Decisión cerrada Auditoría 2 — C3. Detalle en `BOOTSTRAP_PIPELINE.md` §11 + `JSON_SCHEMAS.md` §42 + `MODULES_DEPENDENCY_GRAPH.md` §11.2.
...
Tipo parametric definido en `Verse.digest`: `event<native><public>(t:type)<computes> := class(signalable(t), awaitable(t))`. Implementa las interfaces `signalable(t)` (emitir con `.Signal(payload)`), `awaitable(t)` (esperar con `.Await()`) y `subscribable(t)` (suscribirse con `.Subscribe(handler)`). Es el equivalente Verse de un multicast delegate de Unreal C++. Usado como bloque de construcción del EventBus tipado del proyecto. Fuente: [forums.unrealengine.com — Multicast Delegate Equivalent in UEFN Verse](https://forums.unrealengine.com/t/multicast-delegate-equivalent-in-uefn-verse/1232137).
